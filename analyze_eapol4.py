...
keyinfo byte layout: [0]=1, [1:3]=keyinfo, [3:5]=keylen, [5]=msgnum."""
import struct
...
def main():
    ...
    for i, pkt in enumerate(pkts):
        ...
        if idx == -1:
            print("pkt%03d: no 888e" % i)
            continue
        eapol = pkt[idx+2:]
        if len(eapol) < 6 or eapol[0] != 1:
            print("pkt%03d: bad eapol type=%d len=%d" % (i, eapol[0] if eapol else -1, len(eapol)))
            continue
        ...
        if msg == 1:
            ...
            print("pkt%03d: unknown msg=%d keyinfo=%04x" % (i, msg, keyinfo))
