#!/usr/bin/env python3
"""
Extract 22000 hash from PCAPNG - correct MAC direction
In 802.11: addr1=dst, addr2=src, addr3=BSSID
M1: AP sends to STA, so src=AP, dst=STA
"""
import sys
...
def main():
    ...
    for pkt in pkts:
        ...
        # In 802.11: addr1=dst, addr2=src
        # For M1: AP sends to STA, so src=AP, dst=STA
    ...
    for rsc, group in sorted(rsc_groups.items()):
        ...
        if m1 and m3:
            ...
            print(f"  RSC {rsc}: M1={len(m1)} M3={len(m3)} nonce={nonce_match} M1_src={m1[0]['src_mac']} M1_dst={m1[0]['dst_mac']}")
    ...
    for rsc, group in sorted(rsc_groups.items()):
        ...
        # AP MAC: known (6c4f894ca0e4)
        # STA MAC: source of M1 (Mac sends M3, AP sends M1)
        # In this capture, M1 src=Mac (60f81dad01e4), dst=AP (6c4f894ca0e4)
        # So STA = M1 src
        sta_mac = m1f['src_mac']
        anonce = m1f['nonce']
        key_mic = m3f['mic']
