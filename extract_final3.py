#!/usr/bin/env python3
"""
Extract 22000 hash from PCAPNG - use raw bytes for RSC, scapy for MIC
"""
import sys
...
def main():
    ...
    for pkt in pkts:
        ...
        # Message number from raw bytes
        key_info = int.from_bytes(raw[1:3], 'big')
        msg_num = (key_info >> 8) & 0x03
        msg_counts[msg_num] += 1
        
        # RSC from raw bytes (raw[13:17] = 4 bytes)
        rsc_raw = raw[13:17]
        rsc = rsc_raw.hex()
        
        # MIC from scapy (16 bytes)
        mic = ek.key_mic.hex()
        
        # Nonce from scapy (32 bytes)
        nonce = ek.key_nonce.hex()
