#!/usr/bin/env python3
"""
Extract 22000 hash from PCAPNG using scapy - fixed MAC extraction
"""
import sys
from collections import defaultdict
from scapy.all import rdpcap, EAPOL_KEY, Dot11
...
def main():
    ...
    # Find Dot11 layer
    has_dot11 = sum(1 for p in pkts if p.haslayer(Dot11))
    print(f"\nPackets with Dot11 layer: {has_dot11}/{len(pkts)}")
    
    # Group by RSC, extract MACs from Dot11
    ...
    for pkt in pkts:
        ...
        # Extract MACs from Dot11 layer
        dst_mac = '000000000000'
        src_mac = '000000000000'
        if pkt.haslayer(Dot11):
            d11 = pkt[Dot11]
            dst_mac = str(d11.addr1).replace(':', '')
            src_mac = str(d11.addr2).replace(':', '')
