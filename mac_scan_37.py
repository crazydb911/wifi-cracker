...
# Try with -r (read from file) - use existing capture
print("=== airodump-ng -r /tmp/test (sudo space) ===")
shell = client.invoke_shell()
time.sleep(2)
shell.send("sudo -S /usr/local/Cellar/aircrack-ng/1.7_2/sbin/airodump-ng -r /tmp/test 2>&1 | head -40\n")
time.sleep(3)
shell.send(" \n")  # Space
time.sleep(15)
