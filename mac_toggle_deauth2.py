"""WiFi toggle: airport -z (deauth) + reconnect."""
...
# Start tshark
echo "$SUDO_PASS" | sudo -S /usr/local/bin/tshark -i en0 -I -y IEEE802_11 -c 20000 -w $cap > /tmp/tshark_deauth2.log 2>&1 &
TP=$!
echo "tshark PID: $TP" >> $log
sleep 3

# Use airport -z to deauth (keeps radio on)
echo "airport -z $(date)" >> $log
echo "$SUDO_PASS" | sudo -S $AIRPORT en0 -z 2>&1 >> $log
sleep 5

# Check state
NET=$(networksetup -getairportnetwork en0 2>&1)
echo "After deauth: $NET" >> $log
...
with sftp.open('/tmp/deauth2_toggle.sh', 'w') as f:
    f.write(script)
sftp.chmod('/tmp/deauth2_toggle.sh', 0o755)
sftp.close()
client.close()
...
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect(HOST, port=22, username=USER, pkey=key, timeout=15)
stdin, stdout, stderr = client.exec_command("nohup bash /tmp/deauth2_toggle.sh > /dev/null 2>&1 &", timeout=10)
stdout.read()
client.close()
...
for i in range(25):
    ...
    try:
        ...
        stdin, stdout, stderr = client.exec_command("cat ~/deauth2_cap.log 2>/dev/null | tail -1", timeout=10)
...
try:
    ...
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect(HOST, port=22, username=USER, pkey=key, timeout=15)
    stdin, stdout, stderr = client.exec_command("cat ~/deauth2_cap.log 2>/dev/null", timeout=15)
    out = stdout.read().decode()
    print(f"\n=== LOG ===\n{out}")
    stdin, stdout, stderr = client.exec_command("/usr/local/bin/tshark -r ~/deauth2_cap.pcap -Y 'eapol && (wlan.sa == 6c:4f:89:4c:a0:e4 || wlan.da == 6c:4f:89:4c:a0:e4)' -T fields -e frame.number -e wlan.sa -e wlan.da 2>&1 | head -20", timeout=15)
    out = stdout.read().decode()
    print(f"\n=== EAPOL (32H9F_5G) ===\n{out}")
    client.close()
